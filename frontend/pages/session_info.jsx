...
export default function SessionInfo() {
    ...
    setStudents(prevStudents => 
      prevStudents.map(student => 
        student.id === studentId 
          ? { ...student, message_state: messageState }
          : student
      )
    );
    
    // Temporarily disable auto-refresh to preserve the state update
    // The auto-refresh will resume after the next interval
  };

    // Define fetchStudents outside useEffect so it can be used by the refresh button
  const fetchStudents = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${getApiBaseUrl()}/api/students`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      
      // Update students with fresh data from server
  ...
  useEffect(() => {
    ...
    // Auto-refresh every 10 seconds (less frequent to avoid overriding local updates)
  ...
  const handleFilter = () => {
    ...
    if (selectedGrade) filteredList = filteredList.filter(s => s.grade && s.grade.toLowerCase().includes(selectedGrade.toLowerCase()));
